...
| `next rel <r1> <r2>` | **Relation scan** for obfuscated values stored as `a*real + b` (scaled, offset, negated encodings). Run after `scan unknown` (r1 = the real value when the snapshot was taken, r2 = the real value now) or on any existing result set, RAM or disk-backed (r1 = real value at the last scan/next pass; disk sets are streamed in chunks). **Argument order is chronological: old value first, current value second** ("it was 100, now it's 85" → `next rel 100 85`); swapping them computes wrong `(a, b)` and later refinements will reject the real address. Keeps addresses whose change fits a plausible linear relation (slope must be an integer or 1/integer) and records per-address `(a, b)`. Use the two-arg form **once** to establish, then refine with the one-arg form. |
...
| `results write <idx\|range\|list> <val>` (alias `results w`) | Write a value to result-set entries, encoded with each entry's captured type. |
| `results freeze <idx\|range\|list> <val>` (alias `results f`) | Freeze result-set entries to a value (shows up in `frozen` as `rs[idx]`). |
| `results remove <idx\|range\|list>` (alias `results rm`) | Remove entries from the result set. |
| `results clear` | Empty the result set (deletes `memhacker_results.bin`). The ONLY thing that clears it; new scans and `reset` don't touch it. |
...
results               <- view top rows (results add 5 to stash them for testing)
...
next rel 100 85       <- establish ONCE: old value first, then current
                      <- take more damage, HP now 70
next rel 70           <- refine (one arg from now on): same line, third point
